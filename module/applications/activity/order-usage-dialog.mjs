...
export default class OrderUsageDialog extends ActivityUsageDialog {
  ...
  static PARTS = {
    order: {
      template: "systems/n5eb/templates/activity/order-usage.hbs"
    },
    footer: {
  ...
  _prepareCostsContext(context, { days, gold }) {
    const { duration } = game.settings.get("n5eb", "bastionConfiguration");
    context.costs = {
      days: {
  ...
  async _prepareOrderContext(context, options) {
    ...
    let { duration } = game.settings.get("n5eb", "bastionConfiguration");
