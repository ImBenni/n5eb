...
export default class CalendarSettingsConfig extends BaseSettingsConfig {
  ...
  static PARTS = {
    config: {
      template: "systems/n5eb/templates/settings/base-config.hbs"
    },
    preferences: {
      template: "systems/n5eb/templates/settings/base-config.hbs"
    },
  ...
  async _prepareConfigContext(context, options) {
    const data = game.settings.get("n5eb", "calendarConfig");
  ...
  async _preparePreferencesContext(context, options) {
    const data = game.settings.get("n5eb", "calendarPreferences");
    ...
    } else if ( !game.settings.get("n5eb", "calendarConfig")?.enabled ) {
