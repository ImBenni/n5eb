...
export default class CompendiumBrowserSettingsConfig extends Application5e {
  ...
  static PARTS = {
    sidebar: {
      id: "sidebar",
      template: "systems/n5eb/templates/compendium/sources-sidebar.hbs"
    },
    packs: {
      id: "packs",
      template: "systems/n5eb/templates/compendium/sources-packs.hbs"
    }
  };
    ...
    return {
      ...
      entries: Array.from(packs.map(id => {
        ...
        if ( packageName === "dnd5e" ) {
          tag = flags?.n5eb?.sourceBook?.replace("SRD ", "");
        }
        return {
          tag, title,
          id: collection,
  ...
  async _onToggleSource(target) {
    ...
    const setting = { ...game.settings.get("n5eb", "packSourceConfiguration"), ...packs };
    await game.settings.set("n5eb", "packSourceConfiguration", setting);
    this.render();
  }
