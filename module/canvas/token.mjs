...
export default class Token5e extends foundry.canvas.placeables.Token {
  ...
  findMovementPath(waypoints, options) {
    ...
    if ( (game.settings.get("n5eb", "movementAutomation") !== "full") || !this.document.actor?.system.isCreature
  ...
  _getMovementCostFunction(options) {
    ...
    if ( game.settings.get("n5eb", "movementAutomation") === "none" ) return costFunction;
  ...
  constrainMovementPath(waypoints, options) {
    ...
    ignoreTokens ||= game.settings.get("n5eb", "movementAutomation") !== "full";
