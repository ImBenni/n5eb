...
export default class ChatMessage5e extends ChatMessage {
  ...
  get shouldDisplayChallenge() {
    ...
    switch ( game.settings.get("n5eb", "challengeVisibility") ) {
  ...
  async renderHTML(options={}) {
    ...
    if ( foundry.utils.getType(this.system?.getHTML) === "function" ) {
      ...
      if ( game.settings.get("n5eb", "autoCollapseItemCards") ) {
  ...
  _collapseTrays(html) {
    let collapse;
    switch ( game.settings.get("n5eb", "autoCollapseChatTrays") ) {
  ...
  _enrichAttackTargets(html) {
    ...
    const visibility = game.settings.get("n5eb", "attackRollVisibility");
