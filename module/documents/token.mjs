...
export default class TokenDocument5e extends SystemFlagsMixin(TokenDocument) {
  ...
  getBarAttribute(barName, options={}) {
    ...
    } else if ( ["resources.legact", "resources.legres", "resources.tenacity", "resources.eliteact"]
  ...
  static registerMovementActions() {
    ...
    CONFIG.Token.movement.actions.crawl.getCostFunction = token => {
      const noAutomation = game.settings.get("n5eb", "movementAutomation") === "none";
      const { actor } = token;
  ...
  async _preCreate(data, options, user) {
    ...
    if ( this.actor?.system.isNPC && !this.actorLink
      ...
      const autoRoll = options.dnd5e?.autoRollNPCHP ?? game.settings.get("n5eb", "autoRollNPCHP");
