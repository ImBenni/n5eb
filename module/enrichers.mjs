 ...
 *      data-uuid="Compendium.n5eb.rules.JournalEntry.w7eitkpD7QQTB6j0.JournalEntryPage.UWw13ISmMxDzmwbd"
...
export function createRollLabel(config) {
  ...
  if ( config.icon ) {
    switch ( config.type ) {
      case "check":
      case "skill":
        label = `<i class="dnd5e-icon" data-src="systems/n5eb/icons/svg/ability-score-improvement.svg"></i>${label}`;
        break;
      case "tool":
...
async function handlePostRequest(event, target) {
  ...
  const chatData = {
    ...
    content: await foundry.applications.handlebars.renderTemplate(
      "systems/n5eb/templates/chat/roll-request-card.hbs", { buttons }
    ),
