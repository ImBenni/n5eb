...
/**
 * Register package source books from `flags.n5eb.sourceBooks`.
...
function registerSourceBooks(manifest) {
  if ( !manifest.flags.n5eb?.sourceBooks ) return;
  Object.assign(CONFIG.DND5E.sourceBooks, manifest.flags.n5eb.sourceBooks);
  return "source books";
}
...
/**
 * Register package spell lists from `flags.n5eb.spellLists`.
 * @param {Module|System|World} manifest  Manifest from which to register data.
 * @returns {string|void}                 Description of the data registered.
 */
function registerSpellLists(manifest) {
  if ( !Array.isArray(manifest.flags.n5eb?.spellLists) ) return;
  manifest.flags.n5eb.spellLists.forEach(uuid => dnd5e.registry.spellLists.register(uuid));
  return "spell lists";
}
...
/**
 * Set application based on `flags.n5eb.display`.
 * @param {Compendium} pack  Pack to set up.
 * @returns {string|void}    Description of the step.
 */
function setupPackDisplay(pack) {
  const display = pack.metadata.flags.display ?? pack.metadata.flags.n5eb?.display;
...
/**
 * Set default sorting order based on `flags.n5eb.sorting`.
 * @param {Compendium} pack  Pack to set up.
 * @returns {string|void}    Description of the step.
 */
function setupPackSorting(pack) {
  collectionSortingModes ??= game.settings.get("core", "collectionSortingModes") ?? {};
  if ( !pack.metadata.flags.n5eb?.sorting || collectionSortingModes[pack.metadata.id] ) return;
  collectionSortingModes[pack.metadata.id] = pack.metadata.flags.n5eb.sorting;
  sortingChanged = true;
